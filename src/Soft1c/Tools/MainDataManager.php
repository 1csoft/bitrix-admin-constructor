...
class MainDataManager extends Entity\DataManager
{
	...
	public static function onBeforeAdd(Event $event)
	{
		...
		if($entity->hasField('USER_CREATE')){
			$modifier['USER_CREATE'] = self::getUser()->GetID();
		}

		if($entity->hasField('DATE_X')){
			$modifier['DATE_X'] = new DateTime();
		}
		if($entity->hasField('DATE_CREATE')){
			$modifier['DATE_CREATE'] = new DateTime();
		}
